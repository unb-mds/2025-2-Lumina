...
def mock_text_splitter(mocker):
    ...
    mock_splitter.split_article.return_value = [
        Document(page_content="chunk 1", metadata={"article_id": 1, "url": "http://example.com/1", "title": "Title 1"}),
        Document(page_content="chunk 2", metadata={"article_id": 1, "url": "http://example.com/1", "title": "Title 1"}),
    ]
...
def test_vectordb_init_success(mock_google_embedder, mock_text_splitter, mock_chroma_client):
...
def test_vectordb_init_failure_chroma(mock_google_embedder, mock_text_splitter, mocker):
    ...
    mocker.patch("chromadb.PersistentClient", side_effect=Exception("ChromaDB connection error"))
    with pytest.raises(Exception, match="ChromaDB connection error"):
        VectorDB(embedding_platform=mock_google_embedder, text_splitter=mock_text_splitter)
...
def test_vectorize_article_no_id(
    ...
    with pytest.raises(ValueError, match="Artigo precisa de um ID para ser vetorizado."):
