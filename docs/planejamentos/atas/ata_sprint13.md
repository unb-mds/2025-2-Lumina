...
Planejamento estratégico para a Release 2 e finalização do projeto.
...
* `[Arthur]`: Terminar as documentações do backend
* `[Átila]`: Fazer o bot mandar mensagens mais enxutas
* `[Átila]`: Atualizar o github pages (mudar a função de links importantes no gitpages)
* `[Arthur]`: Fazer o roteiro e slides para a apresentação
* `[Cecília]`: Padronização de linguagem nos testes do backend (para português)
* `[Cecília]`: Fazer realease notes da release 2
* `[Tiago]`: Documentação para decissões arquiteturais
* `[Nathan e Arthur]`: Demonstração das funcionalidades dentro do README
* `[Cecília]`: Texto sobre deploy e integração continua
* `[Cecília]`: Criar um documento falando sobre as funcionalidades dos testes unitários e integração (frontend e backend)
* `[Nathan e João]`: Criar uma versão emergencial do aplicativo caso o deploy não funcione pela playstore
