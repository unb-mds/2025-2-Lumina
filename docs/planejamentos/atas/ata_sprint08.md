# Ata de Reunião - Projeto Lumina

**Data:** `27/10/2025`


**Participantes:**

- Arthur Luiz Silva Guedes

- Átila Sobral de Oliveira

- Cecília Costa Rebelo Cunha

- João Pedro Ferreira Gomes

- Nathan Pontes Romão

- Tiago Geovane da Silva Sousa


---

### 🎯 Objetivo(s) da Reunião:

Sincronizar o status das atividades da semana (Review), identificar pontos de melhoria no processo (Retrospectiva) e planejar os próximos passos para o desenvolvimento do codigo

---


### 📝 Pautas e Definições:


* **Pauta 1: Sincronização Semanal (Review)**

    * **Definição:** Cada membro apresentou o progresso das suas tarefas:

        * **Arthur:** Trabalhou nos scrapers para puxar dados de jornais

	* **Atila:** Trabalhou nos scrapers para puxar dados de jornais

        * **Cecília:** Pesquisou sobre testes unitários.

        * **João:** Trabalhou mais no desenvolvimento do layout da página de configuração.

        * **Nathan:** Trabalhou mais no desenvolvimento do layout da página de configuração.

        * **Tiago:** Criou o pipeline de CD para fazer o deploy automaticamente após mergearem na main


* **Pauta 2: Retrospectiva Semanal**

    * **Definição:** O time realizou a cerimônia de retrospectiva, discutindo os pontos positivos da semana, os desafios encontrados e definindo ações de melhoria para o próximo ciclo de trabalho.


---


### ➡️ Ações / Próximos Passos:

* `[Atila e Arthur e Tiago]`: Trabalhar no spider para puxar os links pro scraper

* `[Atila e Arthur]`Refatorar o codigo dos scrapers para adotar o padrao do código 

* `[Nathan e Joao]`Criar persistencia nas opçoes de configuração do usuario no frontend localmente 

* `[Cecilia]`Criar testes unitarios para o backend 

* `[Nathan e Joao]`Pesquisar como tratar dados de persistencia do historico dos chats no frontend 

* `[Tiago]`Criar/Organizar um banco de dados para receber os dados minerados do webcrawler 

* `[Nathan e Joao]`Pesquisar sobre testes unitários no frontend E2E 
